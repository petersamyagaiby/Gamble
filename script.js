...
const getNumberOfLines = (balance) => {
	while (true) {
		...
		const getNumberOfLine = parseFloat(lines);

		if (isNaN(getNumberOfLine) || getNumberOfLine < 1 || getNumberOfLine > ROWS || getNumberOfLine > balance) {
...
const game = () => {
	...
	while (true) {
		...
		printRows(rowReels);
		const winnings = getWinnings(rowReels, bet, numberOfLines);
